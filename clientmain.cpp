...
static int resolveDest(const std::string &hp, sockaddr_storage &out, socklen_t &len)
{
    ...
    if (rv) { std::cerr << "getaddrinfo: " << gai_strerror(rv) << '\n'; return -1; }
                             ...
                             void *rcvBuf, size_t rcvLen)
{
    for (int attempt = 1; attempt <= MAX_TRIES; ++attempt) {
        ...
        FD_ZERO(&rfds);
        FD_SET(sock, &rfds);
        timeval tv{}; tv.tv_sec = WAIT.count();
...
int main(int argc, char *argv[])
{
    ...
    sockaddr_storage dest{}; socklen_t destLen{};
    if (resolveDest(argv[1], dest, destLen) != 0) return 1;
    std::cout << "Host " << argv[1] << ".\n";

    int sock = socket(dest.ss_family, SOCK_DGRAM, 0);
    if (sock < 0) { perror("socket"); return 1; }
    if (connect(sock, reinterpret_cast<sockaddr*>(&dest), destLen) != 0) { perror("connect"); return 1; }

#ifdef DEBUG
    sockaddr_storage local{}; socklen_t lLen = sizeof(local);
    getsockname(sock, reinterpret_cast<sockaddr*>(&local), &lLen);
    std::cerr << "Connected to " << addrToString(dest) << " local " << addrToString(local) << '\n';
#endif

    calcMessage hello{};
    hello.type = htons(22); hello.message = htonl(0); hello.protocol = htons(17);
    hello.major_version = htons(SUPP_MAJ_VER); hello.minor_version = htons(SUPP_MIN_VER);

    std::aligned_storage_t<sizeof(calcProtocol), alignof(calcProtocol)> rxBuf;
    ssize_t got = txRxWithRetry(sock, &hello, sizeof(hello), &rxBuf, sizeof(rxBuf));
    if (got < 0) { std::cerr << "ERROR: server did not answer within 6 s, giving up.\n"; return 1; }

    if (static_cast<size_t>(got) == sizeof(calcMessage)) {
        auto *cm = reinterpret_cast<calcMessage *>(&rxBuf);
        if (ntohs(cm->message) == 2) { std::cerr << "Server replied NOT OK – protocol version rejected.\n"; return 1; }
        std::cerr << "ERROR WRONG SIZE OR INCORRECT PROTOCOL\n"; return 1;
    }
    if (static_cast<size_t>(got) != sizeof(calcProtocol)) { std::cerr << "ERROR WRONG SIZE OR INCORRECT PROTOCOL\n"; return 1; }

    auto *task = reinterpret_cast<calcProtocol *>(&rxBuf);
    uint32_t arith = ntohl(task->arith);
    int32_t  a = ntohl(task->inValue1); int32_t b = ntohl(task->inValue2);
    double   fa = task->flValue1;      double  fb = task->flValue2;

    const char *opStr = nullptr; double fRes = 0.0; int32_t iRes = 0;
    switch (arith) {
        ...
        default: std::cerr << "Unknown operator from server.\n"; return 1;
    }
    ...
    );  /* semicolon added to close DBG macro call */

    calcProtocol reply{};
    reply.type = htons(2); reply.major_version = htons(SUPP_MAJ_VER); reply.minor_version = htons(SUPP_MIN_VER);
    reply.id = task->id; reply.arith = htonl(arith);
    reply.inValue1 = htonl(a); reply.inValue2 = htonl(b); reply.inResult = htonl(iRes);
    reply.flValue1 = fa; reply.flValue2 = fb; reply.flResult = fRes;

    calcMessage verdict{};
    got = txRxWithRetry(sock, &reply, sizeof(reply), &verdict, sizeof(verdict));
    if (got < 0) { std::cerr << "ERROR: server did not confirm within 6 s, giving up.\n"; return 1; }
    if (static_cast<size_t>(got) != sizeof(calcMessage)) { std::cerr << "ERROR WRONG SIZE OR INCORRECT PROTOCOL\n"; return 1; }

    uint32_t m = ntohl(verdict.message);
    std::cout << (m == 1 ? "OK" : "ERROR") << " (myresult=" << (arith <= 4 ? std::to_string(iRes) : std::to_string(fRes)) << ")\n";
    (void)srvType; (void)addrToString;        /* silence unused-warning */
    return 0;
}
