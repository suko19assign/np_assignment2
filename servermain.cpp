...
struct AddrKeyHash {
    ...
    {
        /* hash the raw bytes – cheap yet fine for small tables */
...
int main(int argc, char* argv[])
{
    ...
    for (;;)
    {
        ...
        if (static_cast<size_t>(got) == sizeof(calcMessage))
        {
            ...
            if (typ != 22 || msg != 0 ||       /* unsupported */
                ntohs(cm->major_version) != SUPP_MAJ_VER ||
                ntohs(cm->minor_version) != SUPP_MIN_VER)
            {   /* Reject */
                calcMessage rej{};
                rej.type          = htons(2);   // server to client, binary
                rej.message       = htonl(2);   // NOT OK
            ...
            /* Send assignment */
            calcProtocol tp{};
            tp.type          = htons(1);          // server to client
